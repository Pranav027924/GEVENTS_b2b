...
export default function InfiniteHorizontalScroll({
  ...
  const updateProgress = useCallback(() => {
    ...
    if (progressBarRef.current) {
      progressBarRef.current.style.transform = `scaleX(${amount})`;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      >
        <div
          ...
          style={{
            height: "100%",
            width: "100%",
            background: "linear-gradient(90deg, #ff6b9d 0%, #c471ed 50%, #12c2e9 100%)",
      ...
      >
        {sections.map((section, idx) => (
          ...
          >
            ...
            {/* Decorative Element - Number/Index */}
            <div
              style={{
                position: "absolute",
                bottom: "6vh",
      ...
      >
        {String(Math.floor(progress * sections.length) + 1).padStart(2, "0")} / {String(sections.length).padStart(2, "0")}
      </div>
    </div>
  );
}